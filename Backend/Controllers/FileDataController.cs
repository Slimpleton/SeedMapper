        ...
        CancellationToken cancellationToken)
        {
            ...
            await using (gzip.ConfigureAwait(false))
            {
                ...
                foreach (var item in filtered)
                {
                    ...
                    if (batch.Count == batchSize)
                    {
                        await JsonSerializer.SerializeAsync(gzip, batch, options: _options, cancellationToken: cancellationToken);
                        await gzip.WriteAsync("\n"u8.ToArray(), cancellationToken);
                        await gzip.FlushAsync(cancellationToken: cancellationToken);
                        batch.Clear();
                        firstBatch = false;
                    }
                }
